...
var produtos = [
    ...
    for(var i=0; i <produtos.length; i++) {
        ...
        template += '<img class="description__image" src="'+produtos[i].imagem[i].src'"alt="'+clientes[i].imagem[i].alt+'">';
        template += '<div class="description-details">'; 
        template += '<h4 class="description-details__title">'+produtos[i].descricao+'</h4>';
        template += ' <div class="description-details__info">';
        template += '<p class="description-details-info__label">Color: </p>'; //como inserir a cor no CSS do ícone
        template += '<i class="fa fa-circle" aria-hidden="true"></i>';
        template += '<p class="description-details-info__label">Size:</p>';
        template += '<p class="description-details__size-selected">'+produtos[i].size+'</p>';
        template += '</div>';
        template += '</div>';
        template += '</td>';
        ...
        template += '<p class="value">'+produtos[i].price+'</p>';
        template += '</div>';
        template += '</td>';
        template += '<td class="item-quantity col-cl-1 col-tb-2 col-dk-2">';
        template += '<input type="number" class="item-quantity__value">'; //como inserir a qtde no input 
        template += '</td>';
        template += '<td class="item-total col-cl-1 col-tb-2 col-dk-2">';
        template += '<div class="item-total__details">';
        template += '<p class="currency">$</p>';
        template += '<p class="value">'+produtos[i].price*+produtos[i].quantity+'</p>';
        template += '</div>';
        template += '</td>';
        template += '</tr>';
    }
